...
from transformers import BertTokenizer, BertForTokenClassification, Trainer, TrainingArguments, TrainerCallback
from torch.utils.data import Dataset, DataLoader
import torch
import matplotlib.pyplot as plt
...
train_dataloader = DataLoader(train_dataset, batch_size=2, shuffle=True, num_workers=8)  # 添加num_workers以提高数据加载速度

training_args = TrainingArguments(output_dir="./Regular Pruning", num_train_epochs=50, per_device_train_batch_size=2, logging_dir='./logs', logging_steps=10)  # 减少epoch数量
