...
from transformers import BertTokenizer, BertForTokenClassification, DistilBertForTokenClassification, Trainer, TrainingArguments, TrainerCallback
from torch.utils.data import Dataset, DataLoader
import matplotlib.pyplot as plt
from torch.nn.utils import prune
import pandas as pd

# 假设数据已提供并加载到dataframe df中
...
# 定义teacher和student模型及tokenizer
teacher_tokenizer = BertTokenizer.from_pretrained("hfl/chinese-roberta-wwm-ext-large")
teacher_model = BertForTokenClassification.from_pretrained("hfl/chinese-roberta-wwm-ext-large", num_labels=len(label_map))
student_tokenizer = BertTokenizer.from_pretrained("distilbert-base-uncased")
student_model = DistilBertForTokenClassification.from_pretrained("distilbert-base-uncased", num_labels=len(label_map))

# 将实体转化为标签
def convert_entities_to_labels(text, entities, tokenizer, label_map):
    tokenized_input = tokenizer(text, truncation=True, padding=True, max_length=256)
...
labels = [convert_entities_to_labels(text, entity["entities"], teacher_tokenizer, label_map) for text, entity in zip(texts, entities)]
...
class NERDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        label_ids = label_ids[:256] + [self.label_map["O"]] * (256 - len(label_ids))  # Ensure label_ids length is 256
...
train_dataset = NERDataset(train_texts, train_labels, teacher_tokenizer, label_map)
eval_dataset = NERDataset(val_texts, val_labels, teacher_tokenizer, label_map)
...
class DistillationTrainer(Trainer):
    ...
    def compute_loss(self, model, inputs, return_outputs=False):
        ...
        student_loss = loss_fct(logits.view(-1, len(label_map)), labels.view(-1))

        with torch.no_grad():
            teacher_outputs = self.teacher_model(**inputs)
            teacher_logits = teacher_outputs.logits

        distil_loss_fct = torch.nn.KLDivLoss(reduction='batchmean')
        distil_loss = distil_loss_fct(F.log_softmax(logits, dim=-1), F.softmax(teacher_logits, dim=-1))
...
class CustomCallback(TrainerCallback):
    ...
    def on_train_end(self, args, state, control, **kwargs):
        ...
        plt.savefig("Distilled_loss_learning_rate_0.02.png")
        plt.show()
...
print(f"Training time per epoch: {(end - start) / training_args.num_train_epochs} seconds")
