...
for epoch in range(1, max_epochs + 1):
    ...
    history = model.fit(train_sentences_padded, train_labels_categorical, batch_size=64, epochs=1, verbose=0)
...
for epoch in range(1, max_epochs + 1):
    ...
    print(f"Training with epoch {epoch}")
    start_time = time.time()
    history = model.fit(train_sentences_padded, train_labels_categorical, batch_size=64 epochs=1, validation_split=0.2, verbose=1)
    training_time = time.time() - start_time
    total_training_time += training_time
    ...
    if val_accuracy > early_stopping_threshold:
        print(f"Early stopping as validation accuracy reached {val_accuracy:.2f}")
