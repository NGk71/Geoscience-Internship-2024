...
import random
# 清空现有数据并添加一个空列表
#with open('crf_train_data.json', 'w', encoding='utf-8') as f:
    #json.dump([], f)
...
# 将新的数据添加到训练数据集中
for text, entity_dict in new_data:
    tokens, labels = tokenize_and_label(text, entity_dict["entities"])
    train_data.append(list(zip(tokens, labels)))

# 随机打乱训练数据顺序
random.shuffle(train_data)

# 将数据转换为新格式
new_train_data = [convert_to_new_format(sentence) for sentence in train_data]

# 保存更新后的训练数据，并确保格式紧凑和分段
with open('crf_train_data.json', 'w', encoding='utf-8') as f:
    f.write('[\n')
    for sentence in new_train_data:
        json.dump(sentence, f, ensure_ascii=False, separators=(',', ':'))
        f.write(',\n')  # 添加逗号和换行以分隔段落
    f.write(']\n')

print("训练数据已更新并保存到 crf_train_data.json 文件中")
