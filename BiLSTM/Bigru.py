...
from tensorflow.keras.layers import Input, GRU, Bidirectional, TimeDistributed, Dense, Embedding
...
model = Bidirectional(GRU(units=100, return_sequences=True, dropout=0.5, recurrent_dropout=0.5))(model)
...
for epoch in range(1, max_epochs + 1):
    ...
    history = model.fit(train_sentences_padded, train_labels_categorical, batch_size=64, epochs=1, verbose=1)
...
if os.path.exists(model_path):  # 确保模型文件存在
    ...
    test_tokens = [token if token in known_tokens else 'O' for token in test_tokens]
